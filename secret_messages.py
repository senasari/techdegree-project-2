...
def create_menu():
    """prompt input for the user (for enc/dec & cipher choice) and store the answers to variables within the method."""
    ...
    print()
    cipher_choice = input('Which cipher would you like to choose? (Please enter the number) ')
...
def encode(cipher_instance, opt, message):
    """based on the answer encrypt or decrypt the message and print the encoded message to the user."""
    encoded = cipher_instance.encrypt(message) if opt == 'E' else cipher_instance.decrypt(message)
    print('Your message is: '+encoded)


def choose(cipher, opt, messg):
    """based on the cipher choice call the necessary class & call the encode method"""
    if cipher == '1':
        af = Affine()
        encode(af, opt, messg)
    elif cipher == '2':
...
# continuous loop of communicating to the user until they say 'nope'
while True:
    inp = input('Anything else you want to decrypt or encrypt? (Nope/type something) ')
    if inp.lower() == 'nope':
        break
    else:
